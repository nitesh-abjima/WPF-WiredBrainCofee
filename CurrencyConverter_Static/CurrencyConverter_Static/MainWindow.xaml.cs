...
namespace CurrencyConverter_Static
{
    public partial class MainWindow : Window
    {
...

        {
            ...
            dtCurrency.Rows.Add("USD", 82.16);
            dtCurrency.Rows.Add("EUR", 92.20);
            dtCurrency.Rows.Add("POUND", 107.72);
            dtCurrency.Rows.Add("JPY", 0.59);
            dtCurrency.Rows.Add("CAD", 58.34);
            dtCurrency.Rows.Add("AUD", 52.17);
        ...
        {
            // Regex pattern to match numeric input
            Regex regex = new Regex("[^0-9]+");

            // Check if the entered text matches the numeric pattern
            if (regex.IsMatch(e.Text))
            {
                // Cancel the input event if it contains non-numeric characters
                e.Handled = true;
            }
        }
